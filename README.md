...
- レイヴンを撤去しても、荷が残っている荷下ろし地点は残る。空になっても自分では消えないので、要らなければ右クリックで回収
...
| Keep a dispatch port that still holds items | `true` | レイヴンを撤去しても、荷が残っている荷下ろし地点を残す。`false` で本体どおり消える |
...
|---|---|---|
| Enabled | `true` | |
| Hide control hints | `true` | 操作説明と選択物の説明を隠す。魔法を構えている間は魔法の枠だけ出る |
