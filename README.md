...
| それ以外 | その位置に荷下ろし地点を設置（隣に入力口があれば、本体のスナップどおりそこへ向く） |

途中に岩・設備・未購入の土地があれば、その手前に寄せて置く。どこにも置けないときだけ止まる

- レイヴンの在庫切れで打ち止め

### 自動配置

![自動配置](img/chain-click.webp)

レイヴンを置いて、範囲外の配送地点をクリック
資源・設備・未購入の土地を避けて、まっすぐ自動で並べる
...
![中間移動](img/map-halfstep.webp)

マップビューの視点移動に「土地の中間」を追加

- 境界で再投下すると、着地点も境界になる
- 境界に止まれるのは両側（角なら4枚）が所有済みの土地のみ
- 境界にいる間は左のUIは非表示
- キーを押しっぱなしで連続移動
...
4. ゲームを起動し、レイヴンを置いて離れた位置をクリックし、線が伸びることを確認する
...
| Stop at tile boundaries | `true` | `false` で本体どおりマスの中央だけ |
...
| | |
|---|---|
| Lazy Witch's Factory | **ver 0.27.0** で動作確認 |
...
| `patches=14` 未満 | **一部の機能が働かない** — ゲームの版が合っていない |
